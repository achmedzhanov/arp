...
{
    public partial class MapElementImpl : XmlTag,  IMapElement
    {
        ...
        private const int CompositeIndex_ROLE_ID = 25;

        private const int IndexManyToMany_ROLE_ID = 26;

        private const int IndexManyToAny_ROLE_ID = 27;

        private const int Element_ROLE_ID = 28;

        private const int OneToMany_ROLE_ID = 29;

        private const int ManyToMany_ROLE_ID = 30;

        private const int CompositeElement_ROLE_ID = 31;

        private const int ManyToAny_ROLE_ID = 32;

        private const int Filter_ROLE_ID = 33;

        private const int Loader_ROLE_ID = 34;

        private const int SqlInsert_ROLE_ID = 35;

        private const int SqlUpdate_ROLE_ID = 36;

        private const int SqlDelete_ROLE_ID = 37;

        private const int SqlDeleteAll_ROLE_ID = 38;

        #endregion

        #region Constructor 

        public MapElementImpl() : base(ElementType.MapElementImpl_ELEMENT) {}
        ...
        public IXmlAttribute SortAttribute 
        {
            get
            {
                return base.GetAttribute("sort");
            }
        }
        ...
        public ICompositeIndexElement CompositeIndex
        {
            get
            {
                return (ICompositeIndexElement)FindChildByRole(CompositeIndex_ROLE_ID);
            }
        }

        public IIndexManyToManyElement IndexManyToMany
        {
            get
            {
                return (IIndexManyToManyElement)FindChildByRole(IndexManyToMany_ROLE_ID);
            }
        }

        public IIndexManyToAnyElement IndexManyToAny
        {
            get
            {
                return (IIndexManyToAnyElement)FindChildByRole(IndexManyToAny_ROLE_ID);
            }
        }
        ...
        public override short GetChildRole(TreeElement child)
        {
                ...
                if (child is ICompositeIndexElement && ((XmlTag)child).TagName == "composite-index")
                    return CompositeIndex_ROLE_ID;
                else 
                
                if (child is IIndexManyToManyElement && ((XmlTag)child).TagName == "index-many-to-many")
                    return IndexManyToMany_ROLE_ID;
                else 
                
                if (child is IIndexManyToAnyElement && ((XmlTag)child).TagName == "index-many-to-any")
                    return IndexManyToAny_ROLE_ID;
                else 
